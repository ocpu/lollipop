...
export async function setupApplictionsForTest(startApplication?: () => Promise<void>): Promise<TestSetup> {
  ...
  return {
    async resolveRequest(...args: [selector: Application | Deno.ListenOptions | string, request: Request] | [request: Request]) {
...
export interface ResponseContext {
  ...
  json(data: unknown, replacer?: ((this: Record<string | number | symbol, unknown>, key: string, value: unknown) => unknown) | undefined): void
...
export function createApp(_middlewares?: ApplicationMiddleware[]): Application {
  ...
  const app: Application = {
    ...
    async serve(...args: [options?: ServeInit] | [address: string, options?: Omit<ServeInit, keyof Deno.ListenOptions>]) {
              ...
              try {
                while (true) {
                  ...
                  while (messageQueue !== undefined && (message = messageQueue.pop()) !== undefined) {
                    ...
                    controller.enqueue(textEncoder.encode(messageData.length.toString(16) + '\r\n' + messageData + '\r\n'))
                  }
                  defering = deferred()
                  await defering
                }
