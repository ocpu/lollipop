...
function childToString(item: JSX.Children): string | BasicElement {
	if (Array.isArray(item)) return item.map(childToString).join('')
	else return item as string | BasicElement
}
...
const React = {
	createElement(
		element:
			| string
			| ((...args: Record<string, unknown>[]) => BasicElement | BasicElement[])
		...
		if (typeof element === 'function') {
			const props = Object.assign(
				{ children },
				Object((element as unknown as Record<string, unknown>).defaultProps),
				Object(properties)
			)
			...
			try {
				...
				if (e instanceof TypeError && e.message.includes('Class constructor')) {
					result = (
						new (element as new (...args: Record<string, unknown>[]) => Record<string, unknown>)(
							Object(properties)
...
declare global {
	namespace JSX {
		...
		type Children =
			...
			| readonly string[]
			| BasicElement
			| readonly BasicElement[]
			| readonly (string | BasicElement)[]
